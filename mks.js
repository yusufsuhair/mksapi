...
app.get('/data', async (req, res) => {
    try {
        ...
        $('table.table-full tbody tr:contains("Gold   : 1 Kilo 999.9 (MYR)")').each((i, elem) => {
            console.log($(elem).find('td.royalblue label').text())
            if (i <= 1) {
                data.oneKilo = $(elem).find('td.royalblue label').text().replace(/\s+/g, '');
            }
        });

        $('table.table-full tbody tr:contains("Gold   : 100 GM 999.9 (MYR)")').each((i, elem) => {
            if (i <= 1) {
                data.oneHundredGram = $(elem).find('td.royalblue label').text().replace(/\s+/g, '');
            }
        });

        $('table.table-full tbody tr:contains("Gold   : 50 GM 999.9 (MYR)")').each((i, elem) => {
            if (i <= 1) {
                data.fiftyGram = $(elem).find('td.royalblue label').text().replace(/\s+/g, '');
            }
        });

        $('table.table-full tbody tr:contains("Gold   : 1 Tael 999.9 (MYR)")').each((i, elem) => {
            if (i <= 1) {
                data.index = $(elem).find('td.royalblue label').text().replace(/\s+/g, '');
            }
        });
